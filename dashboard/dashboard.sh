...
dashboardURL="http://localhost:8001/api/v1/namespaces/kubernetes-dashboard/services/https:kubernetes-dashboard:/proxy/#/login"
...
start)
   ...
   kubectl apply -f ~/dashboard/metrics-server.yaml >/dev/null 2>&1
...
stop)
   ...
   kubectl delete -f ~/dashboard/metrics-server.yaml >/dev/null 2>&1
   echo $msgstopped
   ;;
...
# Show full command line # ps -wfC "$proxy"
if [ $showtoken -gt 0 ]; then
   ...
   echo "URL for dashboard: $dashboardURL"
fi
